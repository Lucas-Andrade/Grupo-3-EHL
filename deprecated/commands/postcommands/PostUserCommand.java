...
public class PostUserCommand extends PostCommand< User >
{
	...
	private static final String[] REQUIREDPARAMETERS = { USERNAME, PASSWORD,
			EMAIL };
	...
	public static class Factory implements CommandFactory
	{
		...
		public Command newInstance( Map< String, String > parameters ) {
			...
			return new PostUserCommand( postingUsersDatabase,
					postedUsersDatabase, parameters );
		}
	 ...
	 *            - Users Data who contains the user responsible to add the new
	 *            user.
	 ...
	 */
	@Override
	protected void internalPostExecute() throws InvalidArgumentException {
		...
		User user = (fullName != null) ? new User( username, password, email,
				fullName ) : new User( username, password, email );
		
		
		User postingUser = usersDatabase.getElementByIdentification( parameters
				.get( "loginName" ) );
		
		
		result = database.add( user, postingUser )
				? "User was successfull added"
