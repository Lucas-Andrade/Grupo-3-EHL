...
/**
 * AirTrafficControl Application for command line. All available commands are resisted first. Then
 ...
 * 
 * <li>"GET /airships/flightId" - return a {@link Airship} info with the given {@code flightId}
 * 
 * <li>"GET /airships/owner/owner" - return all {@link Airship}s info inserted by the {@link User}
 * with the userName {@code owner}
 * 
 * <li>"GET /airships/nbPassengers/nbP/bellow" - return all {@link Airship}s info with passengers
 * number under the given number {@code nbP}
 * 
 * <li>"GET /airships/reports" - return all {@link Airship}s info that are transgressing the
 * {@link AirCorridor}
 * 
 * <li>"GET /airships/reports/flightId" - return a {@link Airship} with the given {@code flightId}
 * info that are transgressing the {@link AirCorridor}
 * 
 * <li>"POST /users username={@value}&password={@value}&email={@value}&fullname={@value}
 * &loginName={@value}&loginPassword={@value}" - create a {@link User}, fullname is optional.
 * 
 * <li>"POST /airships/type latitude={@value}&longitude={@value}&altitude={@value}
 * &minAltitude={@value}&maxAltitude={@value}&
 * {@code airshipCharacteristic}={@value}&loginName={@value}&loginPassword={@value}={@value}
	...
	private static void registerCommands() {
		...
		try {
			...
			parser.registerCommand("GET", "/airships/reports",
					new GetValidationOfAirshipsTransgressionCommand.Factory(airshipDatabase));
			parser.registerCommand("GET", "/airships/reports/{flightId}",
					new GetValidationOfTransgressionByFlightIdCommand.Factory(airshipDatabase));
