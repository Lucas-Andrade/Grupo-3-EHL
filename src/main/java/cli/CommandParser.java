 ...
 * their {@link Callable} instance counterparts.
 * 
 ...
 * registerCommand}; this method must be given the string {@code method}, the
 * string {@code path} (in the formats given above) and a factory that is able
 * to produce a {@link Callable} instance. The {@code path-component}s with
 * format <code>{{@literal<}string>}</code> are placeholders; the placeholder
 * means that, in a concrete command, the text at that position corresponds to
 * the value of a parameter with name. A {@link CommandRegisterException
 * register exception} will be thrown if one of the following two cases happen:
 * <ol>
 * <li>one calls
 * {@link #registerCommand(String, String, StringsToCommandsFactory)
 ...
 * components are both placeholders but are different (e.g. by trying to
 * register two ' {@code GET}'commands with path arguments "
 ...
 * {@code fixed2}" are both placeholders and are different: <code>{ph1}</code>
 * and <code>{ph2}</code>).</li>
 * </ol>
 * </p>
 * <p>
 * After registration of string-commands, one may obtain the {@link Callable}
 ...
 * parameters needed for the factory to produce the {@link Callable} instance.
 * </p>
 * <p>
 * E.g. after calling
 * <ul>
 ...
 * {@link Callable} instance using the given parameter values {@code goodValue}
...
public class CommandParser
{
	...
	private static class Node
	{
		...
		/**
		 * Returns the fixed node child with the {@link #content}
		 * {@code content} if existent; if not returns or {@code null} if not
		 * found.
		 * <p>
	 ...
	 * associated {@link StringsToCommandsFactory factory}. This method must be
	 * given the string {@code method} with the syntax
	 * <p>
	...
	/**
	 * Parses the given strings and produces the corresponding {@link Callable}
	 * instance. The {@code path-component}s with format
	 * <code>{{@literal<}string>}</code> are placeholders. </p> Arguments are
	 * composed of two or three elements. Examples are: POST /products/hotel
	 * strs=5&name=Hotel1&description=hotel
	 ...
	 *            recursion).
	 * @param cmdFactory
	 *            The {@link StringsToCommandsFactory} instance to be stored in
	 *            the last node of the new branch.
	 * @throws InvalidRegisterException
	 *             If the given command cannot be registered (i.e. perhaps the
	 *             grammar is not correct)
	 */
	private void updateSubtree( Node rootNode, String[] methodAndPathElements,
			int pathStartIndex, StringsToCommandsFactory< ? > cmdFactory )
			throws InvalidRegisterException {
		...
		updateSubtree( node, methodAndPathElements, pathStartIndex + 1,
				cmdFactory );
	}
	 ...
	 * registered commands in search for the command factory correspondent to
	 * the string-command whose method and path are stored in the array
	 * {@code methodAndPathElements}. In the way, it completes
	 * {@code parameters} with the names of the placeholders of the registered
	 * command and correspondent values in the string-command. Returns the
	 * instance of {@link Callable} produced by the factory found.
	 * </p>
	 * 
	 * @param rootNode
	 *            The root of the subtree being traversed in the current
	 *            iteration of this method.
	 * @param methodAndPathElements
	 *            The array containing the command's method and path elements.
	 * @param pathStartIndex
	 *            The index of the entry of {@code methodAndPathElements} whose
	...
	private Callable< ? > getCommandInternal( Node rootNode,
		...
		return getCommandInternal( child, methodAndPathElements,
				pathStartIndex + 1, parameters );
	}
