...
/**
 * Class whose instance represent commands to get all the airships in a database that have less than
 * a certain number of passengers. Implements {@code Callable<Optional<Iterable<Airship>>>}.
 *
...
public class GetAirshipsWithLessPassengersThanCommand implements
	 ...
	 */
	public GetAirshipsWithLessPassengersThanCommand(InMemoryAirshipsDatabase database,
			int maximumNumberOfPassengers) throws InvalidArgumentException {
	...
	public Optional<Iterable<Airship>> call() throws Exception {
		...
		return database.getAllElementsThat(new AirshipPredicates.HasPassagersNumberBelowAThreshold(
				max));
	}
}
