...
/**
 * Class whose instance represent commands to get the nearest airships of the
 * specific geographic coordinate. This Class implements
 * {@code Callable<<Iterable<Airship>>>}
 * 
 * @author Daniel Gomes, Eva Gomes, Gonçalo Carvalho, Pedro Antunes
 */
public class GetTheNearestAirshipsOfTheGeographicCoordinateCommand implements
		Callable< Optional< Iterable< Airship > > >
{
	...
	/**
	 * Creates a new instance of this command that gets the airships nearest to
	 * the specific geographic coordinate.
	 * 
	 * @param airshipsDatabase
	 *            The airships database.
	 * @param airshipsNumber
	 *            The number of airships to get nearest to the geographic
	 *            coordinates.
	 * @param latitude
	 *            The latitude geographic coordinate.
	 * @param longitude
	 ...
	 *             if airshipsDatabase are null or airshipsNumber lower than 0
	 *             or latitude it's not between -90 and 90 and longitude is
	 *             higher than 360 and lower than 0.
	 *
	 */
	
	public GetTheNearestAirshipsOfTheGeographicCoordinateCommand(
			InMemoryAirshipsDatabase airshipsDatabase, int airshipsNumber,
			double latitude, double longitude ) throws InvalidArgumentException {
		...
		if( airshipsNumber < 0 )
			throw new InvalidArgumentException(
					"Cannot instantiate command with airship number lower than 0." );
		
		if( !(latitude <= 90 && latitude >= -90) )
			throw new InvalidArgumentException(
					"Cannot instantiate command with latitude higher than 90 and lower than -90." );
		
		if( !(longitude >= 0 && longitude <= 360) )
			throw new InvalidArgumentException(
					"Cannot instantiate command with longitude higher than 360 and lower than 0." );
	...
	/**
	 * Returns a Iterable of airships thats are nearest to the geographic
	 * coordinates
	 * 
	 * <p>
	...
	public Optional< Iterable< Airship >> call() throws Exception {
		...
		return airshipsDatabase.getAirshipsCloserTo( new GeographicPosition(
				latitude, longitude, 0 ), airshipsNumber );
