 ...
 * only during the runtime. </br>Implements {@link Database}.
 * 
 *
		...
		Database< T >
{
	...
	/**
	 * The container of {@link Element}s, where the: <li>keys are the elements'
	...
	// CONSTRUCTOR
	/**
	 * Creates an empty database with name {@code databaseName}. Constructor
	 * used by the subclasses of this class to create an empty database.
	 * 
	...
	public InMemoryDatabase( String databaseName )
		...
		if( databaseName == null )
			throw new InvalidArgumentException(
					"Cannot instantiate database with null name." );
	...
	/**
	 * Stores the {@link Element} {@code element} in this database, added by a
	 * specific {@link User}.
	 * 
	 * @param element
	...
	public boolean add( T element, User user ) throws InvalidArgumentException {
		...
		if( element == null )
			throw new InvalidArgumentException(
					"Cannot add null elements to database." );
		if( user == null )
			throw new InvalidArgumentException(
					"Elements cannot be added by a null user." );
	...
	/**
	 * Removes an element from this database. Implementation of the method
	 * {@link Database#remove() remove()} from the {@link Database} Interface.
	 * 
	 ...
	 *             (e.g removing the user "MASTER" from an
	 *             {@link InMemoryUserDatabase}).
	 * @throws InvalidArgumentException
	...
	public boolean removeByIdentification( String identification )
		...
		if( identification == null )
			throw new InvalidArgumentException(
					"Cannot remove element with null identification." );
	 ...
	 * {@link Optional#get()} throws a {@link NoSuchElementInDatabaseException}
	 * with the message <i>«{@code identification} not found in
	 * {@link #getDatabaseName()}»</i> if the result is {@code null}.
	 * </p>
	 * 
	 * @param identification
	 ...
	 * iterable is "read-only"; attempts to modify it result in an
	...
	/**
	 * Returns an {@link Iterable} with the elements stored in this database
	 * that satisfy the specified {@code criteria}.
	 * <p>
	 * This result is wrapped in an {@link Optional} instance whose method
	 * {@link Optional#get()} throws {@link InternalErrorException} if the
	 * result is {@code null} (since this is never supposed to happen) and whose
	 * method {@link Optional#toString()} returns the string <i>«No such element
	 * in {@code getDatabaseName()}»</i>.
	 * </p>
	 * 
	 * @param criteria
	 *            The criteria to be satisfied by all elements returned.
	 * @return An {@link Iterable} with the elements stored in this database
	 *         that satisfy the specified {@code criteria}.
	 * @throws InvalidArgumentException
