...
{
	/**
	 * Static class that implements {@link Comparator} and whose instances
	 * compare two given {@link Airship}s by its {@link GeographicPosition} in
	 * relation to the {@code GeographicPosition GeographicPositionReference}.
	...
	public static class ComparatorByDistance implements Comparator<Airship>
	{
		...
		public ComparatorByDistance(
				GeographicPosition GeographicPositionReference )
		{
			this.GeographicPositionReference = GeographicPositionReference;
		}

		/**
		 * Compare the two given {@link Airship}s by its
		 * {@link GeographicPosition} in relation to the
		 * {@code GeographicPositionReference}.
		 * 
		 * The {@link GeographicCoordinate Altitude} is not taken in
		 * consideration.
		 * 
		 * @return -1, 0 or 1, when the distance from {@code airship1} to
		 *         {@code GeographicPositionReference} is less than, equal to,
		 *         or greater than the distance from {@code airship2} to
		 *         {@code GeographicPositionReference}
		 */
		@Override
		public int compare( Airship airship1, Airship airship2 )
		{
			double d1 = distanceSquared( GeographicPositionReference,
					airship1.getCoordinates() );
			double d2 = distanceSquared( GeographicPositionReference,
					airship2.getCoordinates() );
