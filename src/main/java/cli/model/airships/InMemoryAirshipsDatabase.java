...
public class InMemoryAirshipsDatabase extends InMemoryDatabase<Airship> {
	...
	/**
	 * by G Problems: Optional not used, IndexOutOfBoundsException extends RuntimeException by E @
	 * G: See if this solves!
	 * 
	 * 
	 * Returns an {@link Iterable} with the {@code nrOfAirshipsToGet} {@link Airship}s stored in
	 * this database that are closer to the {@link GeographicPosition} {@code reference}. The
	 * altitudes are not taken in consideration.
	 * <p>
	 * This result is wrapped in an {@link Optional} instance whose method {@link Optional#get()}
	 * throws {@link InternalErrorException} if the result is {@code null} (since this is never
	 ...
	 * @return an {@link Airship} {@code List} sorted by the distance from each {@code Airship}
	 *         {@link GeographicPosition} to a {@code GeographicPositionReference}.
	 * @throws InvalidArgumentException
	...
	public Optional<Iterable<Airship>> getAirshipsCloserTo(GeographicPosition reference,
		...
		} catch (Exception e) { // never happens because getAll() never has null values
