...
public class ToPlainTextTranslator implements Translator
{
	...
	/**
	 * Returns a string representation of {@code translatable}. <b>It is advised
	 * to only try to encode {@link Translatable} instances that follow the
	 * conventions on the {@link Translatable} documentation.</b>
	 * 
	 * @param translatable
	...
	/**
	 * Returns a string representation of {@code translatable}, which is assumed
	 * to represent an {@code Map<String,String>}.
	 * 
	 * @param translatable
	 *            The {@link Translatable} to be translated.
	 * @return A string representation of {@code translatable}.
	...
	private String encodeMap( Translatable translatable ) {
		...
		StringBuilder sb = new StringBuilder( "__/" )
				.append( translatable.getTag() ).append( "\\__" )
				.append( entryDelimiter ); // prints "---tag---"
		
		for( Entry< String, Object > entry : translatable.getPropertiesBag()
				.entrySet() )
			sb.append( "  " ).append( entry.getKey() ).append( "\r\n" )
					.append( entry.getValue() ).append( entryDelimiter );
		
		
		return sb.append( "\\__" ).append( translatable.getTag() )
				.append( "__/" ).toString();
	}
	
	/**
	 * Returns a string representation of {@code translatable}, which is assumed
	 * to represent an {@link Iterable}.
	 * 
	 * @param translatable
	 *            The {@link Translatable} to be translated.
	 * @return A string representation of {@code translatable}.
	 ...
	 *             specified in the {@link Translatable} documentation.
	 */
	private String encodeIterable( Translatable translatable )
			throws UnknownTranslatableException {
		...
		try
		{
			for( Object element : translatable.getPropertiesBag().values() )
				sb.append( encode( (Translatable)element ) ).append(
						entryDelimiter );
		}
