...
/**
 * Class whose instances represent a command, that have the point to get the info of the
 * {@link CivilAirship}s in {@link InMemoryAirshipDatabase}, that have less passengers than the
 ...
 */
public class GetAirshipsWithANumberOfPassengersBelowACertainThresholdCommand extends
		GetAirshipsCommand {
	...
	public static class Factory implements CommandFactory {
		...
		public Command newInstance(Map<String, String> parameters) {
			...
			return new GetAirshipsWithANumberOfPassengersBelowACertainThresholdCommand(dataBase,
					parameters);
		}
	}
	 ...
	 * {@link GetAirshipsCommand} with the parameters {@code airshipsDatabase} and
	 * {@code parameters}.
	 * 
	...
	/**
	 * Internal execute that will search in the {@code airshipDatabase} for airships with less
	 * passengers than the given {@code minimum passengers number}. The class {@link Predicate} is
	 * used in {@link AirshipPredicates#IsBelowPassagerNumber} method. The {code airship} info is
	 * then inserted in the {@code result} variable from {@link GetAirshipsCommand}
	 */
	@Override
	protected void internalExecute() throws CommandException {

		Long auxLongParameter = Long.parseLong(parameters.get(Param.BELLOW_PASSENGERS_NUMBER));
		result = listToString(
				airshipsDatabase.getAirshipsThat(new AirshipPredicates.HasPassagersNumberBelowAThreshold(
						...
						"There are no Airships with less passengers then {0}", auxLongParameter));
	}

	/**
	 * Returns an array of {@code String}s that has the names of the parameters (to be validate in
	 * {@link AbstractCommand}) without whom the command cannot execute: NUMBERBELLOWPASSENGERS
	 * 
